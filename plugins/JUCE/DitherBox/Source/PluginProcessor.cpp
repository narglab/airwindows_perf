...
//==============================================================================
DitherBoxAudioProcessor::DitherBoxAudioProcessor()
...
//==============================================================================
const juce::String DitherBoxAudioProcessor::getName() const
{
	return JucePlugin_Name;
}

bool DitherBoxAudioProcessor::acceptsMidi() const
{
...
bool DitherBoxAudioProcessor::producesMidi() const
{
...
bool DitherBoxAudioProcessor::isMidiEffect() const
{
	#if JucePlugin_IsMidiEffect
	return true;
	#else
...
double DitherBoxAudioProcessor::getTailLengthSeconds() const
{
	return 0.0;
}

bool DitherBoxAudioProcessor::supportsDoublePrecisionProcessing() const
{
	return true;
}

int DitherBoxAudioProcessor::getNumPrograms()
{
	return 1;   // NB: some hosts don't cope very well if you tell them there are 0 programs,
	// so this should be at least 1, even if you're not really implementing programs.
}

int DitherBoxAudioProcessor::getCurrentProgram()
{
	return 0;
}

void DitherBoxAudioProcessor::setCurrentProgram(int index)
{
}

const juce::String DitherBoxAudioProcessor::getProgramName(int index)
{
	return {};
}

void DitherBoxAudioProcessor::changeProgramName(int index, const juce::String &newName)
{
}

//==============================================================================
void DitherBoxAudioProcessor::prepareToPlay(double sampleRate, int samplesPerBlock)
{
	A = 0.86;
...
void DitherBoxAudioProcessor::releaseResources()
{
...
#ifndef JucePlugin_PreferredChannelConfigurations
bool DitherBoxAudioProcessor::isBusesLayoutSupported(const BusesLayout &layouts) const
{
	#if JucePlugin_IsMidiEffect
...
void DitherBoxAudioProcessor::processBlockTemplate(juce::AudioBuffer<sample_type> &buffer, juce::MidiBuffer &midiMessages)
{
	...
	A = *apvts.getRawParameterValue("mA");
	...
	if (dtype > 11)
	{
		highRes = true; dtype -= 11;
	}
	if (dtype > 11)
	{
		dithering = false; highRes = false;
	}
	//follow up by switching high res back off for the monitoring

	const sample_type *channelReadL = buffer.getReadPointer(0);
	const sample_type *channelReadR = buffer.getReadPointer(1);

	sample_type *channelWriteL = buffer.getWritePointer(0);
	sample_type *channelWriteR = buffer.getWritePointer(1);

	for (int pos = 0; pos < num_samples; ++pos)
	{

		double inputSampleL = channelReadL[pos];
		double inputSampleR = channelReadL[pos];

		if (fabs(inputSampleL) < 1.18e-23) inputSampleL = fpdL * 1.18e-17;
		if (fabs(inputSampleR) < 1.18e-23) inputSampleR = fpdR * 1.18e-17;
		sample_type drySampleL = inputSampleL;
		sample_type drySampleR = inputSampleR;

		if (dtype == 8)
		{
			inputSampleL -= noiseShapingL; inputSampleR -= noiseShapingR;
		}

		if (dithering)
		{
			inputSampleL *= 32768.0; inputSampleR *= 32768.0;
		}
		//denormalizing as way of controlling insane detail boosting
		if (highRes)
		{
			inputSampleL *= 256.0; inputSampleR *= 256.0;
		...
		channelWriteL[pos] = inputSampleL;
		channelWriteR[pos] = inputSampleR;

	}
}

void DitherBoxAudioProcessor::processBlock(juce::AudioBuffer<float> &buffer, juce::MidiBuffer &midiMessages)
{
	processBlockTemplate(buffer, midiMessages);
}

void DitherBoxAudioProcessor::processBlock(juce::AudioBuffer<double> &buffer, juce::MidiBuffer &midiMessages)
{
	processBlockTemplate(buffer, midiMessages);
}

//==============================================================================
bool DitherBoxAudioProcessor::hasEditor() const
{
	return true; // (change this to false if you choose to not supply an editor)
}

juce::AudioProcessorEditor *DitherBoxAudioProcessor::createEditor()
{
	return new DitherBoxAudioProcessorEditor(*this);
}

//==============================================================================
void DitherBoxAudioProcessor::getStateInformation(juce::MemoryBlock &destData)
{
...
void DitherBoxAudioProcessor::setStateInformation(const void *data, int sizeInBytes)
{
...
{
	return new DitherBoxAudioProcessor();
}
